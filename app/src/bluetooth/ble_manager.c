...
static const struct bt_data sd[] = {
	BT_DATA_BYTES(BT_DATA_UUID128_ALL, BT_UUID_NICLA_SERVICE_VAL),
};
...
#if defined(CONFIG_NICLA_SERVICE_SECURITY_ENABLED)
...
static void bt_receive_cb(struct bt_conn *conn, const uint8_t *const data, uint16_t len)
{
	...
	ble_nicla_control_process(data, len);
}

static void bt_subscribe_cb(enum nicla_service_send_status status)
{
	return;
}

static struct nicla_service_cb nicla_cb = {
...
#endif /* CONFIG_NICLA_SERVICE_SECURITY_ENABLED */
...
{
	return bt_gatt_is_subscribed(current_conn, bt_gatt_find_by_uuid(NULL, 1, BT_UUID_CONTROL),
...
static void bt_ready_cb(int err)
{
	if (!err) {
		...
		LOG_ERR("Bluetooth initialisation failed (%i)", err);
	}
}
...
uint8_t ble_manager_init()
{
	...
	if (IS_ENABLED(CONFIG_NICLA_SERVICE_SECURITY_ENABLED)) {
	...
	while (!_is_ble_enabled) {
		LOG_WRN("waiting for BLE initialization...");
		k_msleep(100);
	}
	...
	err = nicla_service_init(&nicla_cb);
	if (err) {
		LOG_ERR("Failed to initialize NICLA service (err: %d)", err);
		return 0;
	}
	return 1;
}
